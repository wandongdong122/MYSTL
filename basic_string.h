...
struct char_traits
{
	...
	{
		MYSTL_DEBUG(src + n <= dst || dst + n <= src); // 内存出现重合
		char_type* r = dst;
		for (; n != 0; --n, ++dst, ++src)
			*dst = *src;
		return r;
...
class basic_string
{
	...
	allocator_type get_allocator() { return allocator_type(); } // 构造一个内存构造器对象 
...
public:
	...
	template <class Iter, typename std::enable_if<
		mystl::is_input_iterator<Iter>::value, int>::type = 0> // 这里的 = 0 是初始化？
...
public:
	...
	{ return static_cast<size_type>(-1); }  // 将-1转换为无符号整型

	void     reserve(size_type n);
	void     shrink_to_fit(); // 缩减为了容量与大小的匹配
	...
	template <class Iter, typename std::enable_if<
		mystl::is_input_iterator<Iter>::value, int>::type = 0>  // enable_if 做类型检查，如果迭代器不是输入迭代器则跳过本次匹配，不报错
	...
	// 这里的第二个参数是一个匿名类型参数，即不声明变量名，只是用于做类型匹配
	template <class Iter, typename std::enable_if<
	...
	// find_first_of  返回在 字符串中首次出现的匹配str中的任何一个字符的首字符索引
	...
	// find_last_not_of 从后往前找首次不匹配的字符
...
// 移动赋值操作符----直接将当前对象的指针指向另一个对象，不经过拷贝构造等“复制”操作
template <class CharType, class CharTraits>
basic_string<CharType, CharTraits>&
basic_string<CharType, CharTraits>::
operator=(basic_string&& rhs) noexcept
{
	destroy_buffer(); // 析构当前对象
	buffer_ = rhs.buffer_;
	size_ = rhs.size_;
	cap_ = rhs.cap_;
	rhs.buffer_ = nullptr;
...
reserve(size_type n)
{
	if (cap_ < n)
	{
		THROW_LENGTH_ERROR_IF(n > max_size(), "n can not larger than max_size()"
			"in basic_string<Char,Traits>::reserve(n)");  // 抛出异常，不处理
		auto new_buffer = data_allocator::allocate(n);
		char_traits::move(new_buffer, buffer_, size_);  // 直接使用memmove 将从buffer_开始的size_个空间移动到new_buffer
		buffer_ = new_buffer;
		cap_ = n;
	}
}
...
{
	auto n1 = mystl::min(count1, size_ - pos1); // 判断剩余长度与count1的大小关系
...
// 从下标 pos 开始查找字符串 s 其中的一个字符出现的第一个位置------字符串s在原字符串中从pos位置开始第一个出现的字符
template <class CharType, class CharTraits>
typename basic_string<CharType, CharTraits>::size_type
basic_string<CharType, CharTraits>::
