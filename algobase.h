...
{
#ifdef max
#pragma message("#undefing marco max")  // 在编译阶段发送信息
#undef max
#endif // max
...
unchecked_copy_cat(InputIter first, InputIter last, OutputIter result,
	mystl::input_iterator_tag /*函数重载标记*/)
{
...
{
	return unchecked_copy_cat(first, last, result, iterator_category(first)/*返回迭代器类型*/);
}

// 为 trivially_copy_assignable 类型提供特化版本
template <class Tp, class Up>
typename std::enable_if< /*满足条件时类型有效*/
	...
	unchecked_move(Tp* first, Tp* last, Up* result)
{
	...
	if (n != 0)
		std::memmove(result, first, n * sizeof(Up));  // 直接将n大小的源内存地址的内容拷贝到目标内存的地址中
	return result + n;
}
