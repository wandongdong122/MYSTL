...
public:
	...
	allocator_type get_allocator() { return data_allocator(); }  // 返回空间配置器
...
public:
	...
	explicit vector(size_type n) // 构造函数，分配n个空间
	{ fill_init(n, value_type()); }

	vector(size_type n, const value_type& value)  // 构造函数 初始值为n个value
	{ fill_init(n,value); }

	template <class Iter,typename std::enable_if< /*std::enable_if 就是满足条件时类型是有效的*/
	...
	vector(vector&& rhs/*当rhs对象为临时对象时调用*/) noexcept  
		:begin_(rhs.begin_),
		end_(rhs.end_),
		cap_(rhs.cap_)
	{
	...
	vector(std::initializer_list<value_type> ilists)
	{
		range_init(ilists.begin(), ilists.end());
	}
...
public:
	...
	{ return begin_; }
	const_iterator         begin()   const noexcept /*const 加在非静态成员函数后面表示这个函数隐含传入的this为const指针，也就是说this指针指向的数据无法修改*/
	{ return begin_; }
	...
	void      reserve(size_type n);  //容器预留len个元素长度，预留位置不初始化，元素不可访问。
	void      shrink_to_fit();      // 请求容器降低其容量和size匹配。具体需要由编译器决定，此时只是做出请求
	...
	// assign
	void assign(size_type n, const value_type& value)  // 设置向量中前n个元素的值为value
	{ fill_assign(n,value); }
	...
	template <class... Args>
	iterator emplace(const_iterator pos, Args&& ...args);  //emplace函数在容器中直接构造元素，传递给emplace函数的参数必须与元素类型的构造函数相匹配。
	...
	{ emplace_back(mystl::move(value)); } // 先把左值强制转换为右值，然后进行插入
	...
	void    insert(const_iterator pos, Iter first, Iter last)
	{
		...
		copy_insert(const_cast<iterator>(pos), first, last);  //const_cast是一种C++运算符，主要是用来去除复合类型中const和volatile属性（没有真正去除）。
	}
...
vector<T>& vector<T>::operator=(const vector& rhs)
{
	if (this != &rhs)
	{
		...
		if (len > capacity())  // 当vector容量小于赋值的vector
		{
			vector tmp(rhs.begin(), rhs.end()); // 创建临时对象
			swap(tmp); // 交换空间
		}
		else if(size() >= len)
		{
		...
		{
			// vector有剩余容量，但需要size()的大小不够
			mystl::copy(rhs.begin(), rhs.begin() + size(), begin_);  // 先将存在的内存复制过去
			mystl::uninitialized_copy(rhs.begin() + size(), rhs.end(), end_); // 构造新的对象
...
void vector<T>::reserve(size_type n)
{
	if (capacity() < n)
	{
		...
		const auto olde_size = size();
		auto tmp = data_allocator::allocate(n);  // 开辟空间
...
vector<T>::emplace(const_iterator pos, Args&& ...args)
{
	...
	if (end_ != cap_ && xpos == end_)// 说明还有剩余空间
	{
		data_allocator::construct(mystl::address_of(*end_), mystl::forward<Args>(args)...);
		++end_;
	}
	else if(end_ != cap_) // xpos != end_
	{
		auto new_end = end_;
		data_allocator::construct(mystl::address_of(*end_), *(end_ - 1));
		++new_end;
		mystl::copy_backward(xpos, end_ - 1, end_); // 将pos-（end-1）处的数据向后移动
		*xpos = value_type(mystl::forward<Args>(args)...); // 将新的元素放在pos处
		//end_ = new_end;
	}
	else
	{
		reallocate_emplace(xpos, mystl::forward<Args>(args)...); // 重新分配内存
	}
	return begin() + n;
}
...
copy_assign(FIter first, FIter last, forward_iterator_tag)
{
	...
	{
		auto new_end = mystl::copy(first, last, begin_);  // 返回的时复制后的位置
	...
	{
		auto mid = first;
		mystl::advance(mid, size()); // 让迭代器前进n个距离
...
reallocate_emplace(iterator pos, Args&& ...args)
{
	...
	try
	{
		new_end = mystl::uninitialized_move(begin_, pos, new_begin); // 将[begin_,pos)上的元素移动到new_begin
