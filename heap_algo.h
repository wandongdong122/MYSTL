...
namespace mystl
{
    ...
    void adjust_heap(RandomIter first, Distance holeIndex, Distance len, T value)
    {
        ...
        while (rchild < len)
        {
            if (*(first + rchild) < *(first + rchild - 1)) // 比较左孩子和右孩子哪个大，将大的与父节点交换
                --rchild;
            *(first + holeIndex) = *(first + rchild);
            holeIndex = rchild;
            rchild = 2 * (rchild + 1);  // 一直下溯到叶子节点
        }
        if (rchild == len)
        {  // 如果没有右子节点
    ...
    void make_heap_aux(RandomIter first, RandomIter last, Distance*)
    {
        if (last - first < 2)
            return;
        auto len = last - first;
        auto holeIndex = (len - 2) / 2;  // 减掉 2 是为了去掉 end 所在的元素  ----返回的是最后一个叶子结点的父节点
        while (true)
        {
            // 重排以 holeIndex 为首的子树
