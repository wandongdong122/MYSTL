...
public:
  ...
  /*
    noexcept 关键字表示函数不会发生异常，在C++中异常是在函数运行时进行检测的，使用关键字可以使得编译器对函数进行优化（不产生额外的异常检测代码）
  */
  void swap(stack& rhs) noexcept(noexcept(mystl::swap(c_, rhs.c_))) // 表明swap(c_, rhs.c_)不发生异常则函数不发生异常
