...
// 获取对象地址
template <class Tp>
constexpr Tp* address_of(Tp& value) noexcept/*不报告异常*/
{
...
// 获取  / 释放 临时缓冲区
template <class T>
pair<T*, ptrdiff_t> get_buffer_helper(ptrdiff_t len, T*) // 需要获取临时缓冲区的大小和类型
{
    ...
    return pair<T*, ptrdiff_t>(nullptr, 0); // 申请失败
}
