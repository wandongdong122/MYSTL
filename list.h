...
struct node_traits
{
	typedef list_node_base<T>* base_ptr;   // 指向 ？？？ 的指针
	typedef list_node<T>*	   node_ptr;   // 指向节点的指针
};

 // list 的节点结构
template <class T>
struct list_node_base           // 存储指针域
{
	...
	list_node_base() = default;  // 定义一个默认构造函数
...
template <class T>
struct list_node : public list_node_base<T>   // 存储数据域
{
  typedef typename node_traits<T>::base_ptr base_ptr;
  typedef typename node_traits<T>::node_ptr node_ptr;
...
// list的迭代器设计  -----与vector不同的是 list的迭代器一个结构体类型
template<class T>
...
public:
  // 构造、复制、移动、析构函数
  list() 
  { fill_init(0, value_type()); }  // value_type()是类型的默认构造函数  比如 int a = int(); cout << a<< endl; 输出0
...
public:
  ...
  { return node_->next; } // 是环形链表，末尾节点的下一个节点便是头节点，并且末节点不存放数据
  const_iterator         begin()   const noexcept
  { return node_->next; }
  iterator               end()           noexcept 
  { return node_; }  // node_指向尾节点
  ...
  template <class ...Args>
  void     emplace_front(Args&& ...args)  // 在前面直接构造一个元素
  {
  ...
  template <class ...Args>
  void     emplace_back(Args&& ...args)  // 在尾部直接构造一个元素
  {
    THROW_LENGTH_ERROR_IF(size_ > max_size() - 1, "list<T>'s size too big");
    auto link_node = create_node(mystl::forward<Args>(args)...);
  ...
  void     resize(size_type new_size) { resize(new_size, value_type()); }  // 以0作为初始化
  ...
  void merge(list& x)  // 合并
  { merge(x, mystl::less<T>()); }
  template <class Compare>
  void merge(list& x, Compare comp);  // comp是谓词
...
void list<T>::resize(size_type new_size, const value_type& value)
{
  ...
  if (len == new_size)  //  新的容器大小小于原有容器大小
  {
    erase(i, node_);
  }
  else // 新的容器大小大于原有容器大小
...
// 移除 list 中满足二元操作 pred 为 true 重复元素
template <class T>
template <class BinaryPredicate>
...
{
  node_ptr p = node_allocator::allocate(1);  // 分配内存
  try
  {
...
{
  auto i = begin();  //  返回指向头结点的迭代器
  auto e = end();    //  返回指向尾结点的迭代器
  for (; n > 0 && i != e; --n, ++i)
  {
    *i = value;
  }
