...
## hashtable的 迭代器
&emsp;&emsp;hashtable的迭代器没有后退操作（operator（）--），hashtable也没有定义所谓的逆向迭代器（reverse iterator）。
## hashtable的 数据结构
&emsp;&emsp;虽然开链法并不要求表格大小必须为质数，但STL任然以质数来设计表格大小，并且先将28个质数计算好。  

&emsp;&emsp;hashtable无法处理 string、 double、float等型别的数据，如果想要处理这些类型的数据，用户需要自行定义hash函数。

## 3.2.5 hash_set
&emsp;&emsp;SGI在STL标准规则之外另提供了一个所谓的hash_set，以hashtable为底层机制。以RB-tree为底层的set有自动排序功能，而以hashtable为底层的没有。所以set的元素有自动排序功能而hash_set没有。
## 3.2.6 hash_map
## 3.2.7 hash_multiset
## 3.2.8 hash_mulmap
