...
{
#ifdef max
#pragma message("#undefing marco max") //编译期间，将一个文字串（messagestring）发送到标准输出窗口
#undef max
#endif // max
...
struct deque_iterator : public iterator<random_access_iterator_tag, T>
{
  ...
  deque_iterator() noexcept
	...
	difference_type operator-(const self& x) const
	{
		...
		// 每个buffer中有多少个元素 * 一共有多少node（buffer） + 最后一个buffer中元素的个数(可能有剩余) - 一个buffer中元素的个数
	}
...
public:
	...
	typedef pointer*	 /* T** */					 map_pointer;
	typedef const_pointer*		/* T** */			 const_map_pointer;
...
public:
	...
	// emplace_front / emplace_back / emplace  ---就地构造
...
deque<T>& deque<T>::operator=(const deque& rhs)
{
	if (this != &rhs)
	{
		...
		if (len >= rhs.size())
		{
			erase(mystl::copy(rhs.begin_, rhs.end_, begin_), end_);  // copy函数返回的是拷贝后的end
		}
		else
		{
			iterator mid = rhs.begin() + static_cast<difference_type>(len);
			mystl::copy(rhs.begin_, mid, begin_);  // copy函数直接使用memmove
...
// 减小容器容量
template <class T>
void deque<T>::shrink_to_fit() noexcept // 将map的大小缩小至size()
{
	// 至少会留下头部缓冲区
...
void deque<T>::emplace_front(Args&& ...args)
{
	...
	{
		data_allocator::construct(begin_.cur - 1, mystl::forward<Args>(args)...);  // begin_.cur永远指向第一个元素
		--begin_.cur;
	}
	else
	{
...
// insert_aux 函数
template <class T>
template <class... Args> // Args 是一个模板参数包，其中可能包含了零个或多个模板参数  // 可以通过sizeof(Args)获取参数包中的参数个数
typename deque<T>::iterator
deque<T>::
insert_aux(iterator position, Args&& ...args) // args 是一个函数参数包，其中可能包含了零个或多个函数参数 // 可以通过sizeof(Args)获取参数包中参数的个数
{
	/*在position位置插入，需要将[begin_ position]位置的内容向前移动，或者把[position end_]位置的内容向后移动*/
  const size_type elems_before = position - begin_;
  value_type value_copy = value_type(mystl::forward<Args>(args)...);  //  C++通过在模式的右边加上一个省略号来触发包扩展
  // 这里的 value_type 相当于 T，因此实际上是一个类型 T 的构造函数
  if (elems_before < (size() / 2))
  {
	// 在前半段插入
    emplace_front(front());  // 在 begin_ 之前构造出一个元素，用 begin_ 所在元素的值作为初始化
    auto front1 = begin_;
    ++front1;               // 原来 begin_ 的位置
    auto front2 = front1;	// 指向原来 begin_ 的位置
    ++front2;               // 指向原来 begin_ 的下一个位置
    position = begin_ + elems_before;
    auto pos = position;
    ++pos;
    mystl::copy(front2, pos, front1); // 将从 原来 begin_ 的下一个位置的 elems_before 个元素向前移动
  }
  else
  { // 在后半段插入
    emplace_back(back());
...
fill_insert(iterator position, size_type n, const value_type& value)
{
  ...
  {
    require_capacity(n, true);  // 向前扩展 n 个元素空间
    // 原来的迭代器可能会失效
  ...
  {
    require_capacity(n, false);  // 向后扩展 n 个元素空间
    // 原来的迭代器可能会失效
    auto old_end = end_;
    auto new_end = end_ + n;
